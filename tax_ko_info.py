                ...
                for level_2 in level_1["children"]:
                    ...
                    try:
                        ...
                        # final print
                            ...
                            kegg_dict[ko] = {"level_1": level_1["name"],'level_2': level_2['name'], 'level_3': func}
                    except KeyError:
                        ko='ko:KO'+level_2['name'].partition(' ')[0]
                        kegg_dict[ko] = {"level_1": level_1["name"],'level_2': level_2['name'], 'level_3': 'NA'}
...
# path='/Users/zavo603/Documents/Nicki_files/perseq/tables/'
    tax_levels={'Super Kingdom': 1,'Kingdom': 1,'Phylum': 2 ,'Class':3,'Order': 4,'Family': 5,'Genus': 6,'Species': 7}
    ...
    for f in os.listdir(path):
        ...
        with open(f) as file:
            ...
            if grouped_sample_tbl is None:
                ...
                full_tbl=pd.read_table(f,usecols=[function,'tax_classification','aa_alignment_length','aa_percent_id']) #grab only the two columns of interest(can be changed later)

                full_tbl['Count_%s'% sample]='NA' #add an empty column
                full_tbl['tax_classification']=tax_ko_tbl['tax'] ##this will assign the tax classification to the edited taxonomy from above
                ...
                grouped_sample_tbl=final_tbl.groupby(['tax_classification',function]).count().reset_index()
                continue

            print(sample)
            tax_ko_tbl=pd.DataFrame(tax_ko)
            full_tbl=pd.read_table(f,usecols=[function,'tax_classification','aa_percent_id','aa_alignment_length']) #grab only the two columns of interest(can be changed later)
            full_tbl['Count_%s'% sample]='NA' #add an empty column
            full_tbl['tax_classification']=tax_ko_tbl['tax'] ##this will assign the tax classification to the edited taxonomy from above
            final_tbl=full_tbl.fillna('NA')
            final_tbl=final_tbl[final_tbl['aa_percent_id'] > min_perc_id]
            final_tbl=final_tbl[final_tbl['aa_alignment_length'] > min_len]
            final_tbl=final_tbl.drop(['aa_percent_id','aa_alignment_length'],axis=1)
            grouped_tbl=final_tbl.groupby(['tax_classification',function]).count().reset_index()
            grouped_sample_tbl = grouped_sample_tbl.merge(grouped_tbl, on=['tax_classification','ko'], how="outer").fillna('0')
    grouped_sample_tbl=grouped_sample_tbl.rename(columns = {'tax_classification':'Tax. Classification(%s)'% tax_level,'ko':'KO'})
