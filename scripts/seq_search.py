...
def dump_hmm_matches(hmmscan_output, hits_file):
    ...
    for qn, (name, elapsed, hits, querylen, seq) in enumerate(
        ...
        last_time = time.time()
        if qn and (qn % 25 == 0):
            print(qn + 1, total_time, "%0.2f q/s" % (float(qn + 1) / total_time))
            # sys.stderr.flush()
    ...
    print(
        ...
        % (qn + 1, elapsed_time, "%0.2f q/s" % ((float(qn + 1) / elapsed_time))),
        "lblue",
    )
...
def parse_hmmscan(hmmscan_output, evalue_thr=None, score_thr=20, max_hits=1):
    ...
    with open(hmmscan_output) as file:
        for line in file:
            ...
            if (
                (evalue_thr is None or evalue <= evalue_thr)
                and (score_thr is not None and score >= score_thr)
                and (
                    max_hits is None
                    or last_hitname == hitname
                    or len(hit_ids) < max_hits
                )
            ):

                hit_list.append(
                    [hitname, evalue, score, hmmfrom, hmmto, seqfrom, seqto, d_score]
                )
                hit_ids.add(hitname)
...
def refine_matches(fasta_file, refined_file, parsed_hmm_output, og2levels):
    refine_header = list(
        map(
            str.strip,
            """#query_name, best_hit_eggNOG_ortholog,
        ...
        for qn, r in enumerate(
            ...
            if qn and (qn % 25 == 0):
                ...
                print(
                    qn + 1,
                    total_time,
                    "%0.2f q/s (refinement)" % ((float(qn + 1) / total_time)),
                )
                # sys.stderr.flush()
            query_name = r[0]
            best_hit_name = r[1]
...
def process_nog_hits_file(
    ...
    with open(parsed_hmm_output) as file:
        for line in file:
            ...
            target_fasta = (
                "/Users/zavo603/anaconda3/envs/python2/lib/python2.7/site-packages/data/OG_fasta/%s/%s.fa"
                % (level, hitname)
            )
...
def get_best_hit(target_seq, target_og, excluded_taxa):
    ...
    # PHMMER= 'phmmer'
    cmd = (
        "/Users/zavo603/anaconda3/bin/phmmer --incE 0.001 -E 0.001 -o rando.txt --noali --tblout %s %s %s"
...
def main(hmmscan_output, hits_file, translated_fasta, refinement_output, og2levels):
    ...
    refined_matches = refine_matches(
        translated_fasta, refinement_output, hits_file, og2levels
    )


if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument("--hmmscan_output", type=str, help="hmmscan output file")
    ...
    p.add_argument("--refinement_output", type=str, help="output file for refinement")
    p.add_argument("--og2levels", type=str, help="og2levels file from eggnog db")
