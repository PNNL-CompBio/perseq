...
def annotate_hits_file(seed_orthologs_file, annot_file, hmm_hits_file):
    ...
    for result in pool.imap(annotate_hit_line, iter_hit_lines(seed_orthologs_file)):
        ...
        if result:
            ...
            OUT.write('\t'.join(map(str, (query_name,
                                             best_hit_name,
                                             best_hit_evalue,
                                             best_hit_score,
                                             best_name,
...
def annotate_hit_line(arguments):
    ...
    if orthologs:
        pname, gos, kegg, bigg = summarize_annotations(orthologs,
                                                         target_go_ev="non-electronic",
                                                         excluded_go_ev=set(["ND", "IEA"]))
...
def connect():
    print('using connect')
    global conn, db
    conn = sqlite3.connect("/Users/zavo603/anaconda3/envs/python2/lib/python2.7/site-packages/data/eggnog.db")
    db = conn.cursor()
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--search_output",
        type=str,
        help="output file of seq_search.py",
    )
    p.add_argument("anot_file", type=str, help="annotation ouptut file name")
    p.add_argument(
        "--hits_file", type=str, help="hmm hits file returned from hmmscan"
    )
